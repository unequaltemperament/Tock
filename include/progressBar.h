...
public:
  ...
  void update(bool forceUpdate = false) {
    ...
    if ((currentMillis - updatedAt >= lightIntervalInMs && currentTimer->remainingTimeInMS > 0) || forceUpdate) {
      if(!forceUpdate){updatedAt = currentMillis;}
      double fullLEDsInt;
      double partial = modf(((double)currentTimer->remainingTimeInMS * _num_leds / currentTimer->initialTimeInMS) ,&fullLEDsInt);
...
private:
  ...
  int getMappedLED(int realID) {
    ...
    assert(realID >= 0 && realID < _num_leds);
